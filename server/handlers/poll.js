...
exports.showPolls = async (req, res, next) => {
    try {
        const polls = await db.Poll.find(
            {
                expireDate: { $gte: new Date(Date.now()).toISOString() },
                privatePoll: false
            }
...
exports.userHistory = async (req, res, next) => {
    try {
        ...
        let user = await db.User.findById(id).populate({
            path: "polls",
            match: { expireDate: { $lte: new Date(Date.now()).toISOString() } },
        });
...
exports.createPoll = async (req, res, next) => {
    try {
        ...
        const { question, options, expireDate, privatePoll, multipleChoice } = req.body;
...
exports.vote = async (req, res, next) => {
    try {
        ...
        if (answer) {
            ...
            const active = await db.Poll.findById(pollId).find({
                expireDate: { $gte: new Date(Date.now()).toISOString() },
            });
